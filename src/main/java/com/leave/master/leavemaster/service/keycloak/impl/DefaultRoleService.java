...
public class DefaultRoleService extends AbstractKeycloakService implements RoleService {
    ...
    public void syncRoles() {
        syncRoles(Role.findAllByAccess("leavemaster"), properties.getKeycloak().getClientId());
    }
                        ...
                        roleChanges ->
                                Try.run(
                                        () -> {
                                            ...
                                            manageRoles(clientId, roleChanges.rolesToAdd(), roleChanges.rolesToRemove(),true);
                                        }))
    ...
    private void syncRolesForRealm(final Set<String> roles, final String clientId) {
         Try.of(() -> roles)
                .flatMap(newRoles -> findRealmRoles()
                        .map(existingRoles -> createRoleChanges(newRoles, existingRoles))
                )
                .flatMap(roleChanges -> Try.run(() -> {
                    manageRoles(clientId, roleChanges.rolesToAdd(), roleChanges.rolesToRemove,false);
                    roleChanges.rolesToAdd().forEach(role ->
                            associateRealmRoleWithClientRole(role.getRoleName(), clientId, role.getRoleName())
                    );
                }))
    ...
    private void associateRealmRoleWithClientRole(String realmRoleName, String clientId, String clientRoleName) {
        getRealmResource()
                .onSuccess(resource -> {
                    ClientRepresentation client = findClient(resource, clientId);
                    String clientUuid = client.getId();
                    RoleRepresentation clientRole = resource.clients().get(clientUuid).roles().get(clientRoleName).toRepresentation();
                    resource.roles().get(realmRoleName).addComposites(List.of(clientRole));
                    log.info("Associated realm role '{}' with client role '{}'", realmRoleName, clientRoleName);
                })
                .onFailure(e -> log.error("Failed to associate realm role '{}' with client role '{}': {}", realmRoleName, clientRoleName, e.getMessage()));
    }
    ...
    private void manageRoles(String clientId, Set<Role> rolesToAdd, Set<String> rolesToRemove, boolean isClient) {
    ...
    private RoleRepresentation createClientRoleRepresentation(final String clientId, final Role role) {
    ...
    private void createRole(
        ...
        Try.run(() -> {
               var client =realmResource.clients().findByClientId(clientId).stream()
                                            .findFirst()
                                            .orElseThrow(() -> new IllegalStateException("Client with clientId '" + clientId + "' not found."));
                ...
                .onFailure(e -> log.error("Failed to create role '{}': {}",roleRepresentation.getName(), e.getMessage()));
    }
    ...
    private Try<Set<String>> findRealmRoles() {
        return getRealmResource().map(RealmResource::roles)
                .map(RolesResource::list)
                .map(roleRepresentations -> roleRepresentations.stream()
                        .map(RoleRepresentation::getName)
                        .collect(Collectors.toSet()));
