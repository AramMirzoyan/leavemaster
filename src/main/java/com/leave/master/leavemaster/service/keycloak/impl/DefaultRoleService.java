...
public class DefaultRoleService extends AbstractKeycloakService implements RoleService {
  ...
  public void syncRoles() {
    syncRolesForClient(Role.findAllByAccess("leavemaster"), properties.getKeycloak().getClientId())
        .onFailure(e -> log.error("Failed to sync roles", e));
  }

  private Try<Void> syncRolesForClient(final Role[] roles, final String clientId) {
    return Try.of(() -> prepareRoles(roles))
            ...
            roleChanges ->
                Try.run(
                    () -> {
                      ...
                      manageRoles(clientId, roleChanges.rolesToAdd(), roleChanges.rolesToRemove());
                    }))
  ...
  private void manageRoles(String clientId, Set<Role> rolesToAdd, Set<String> rolesToRemove) {
  ...
  private void addRolesToClient(String clientId, Set<Role> rolesToAdd) {
    rolesToAdd.forEach(
        role -> {
          RoleRepresentation roleRepresentation = createRoleRepresentation(clientId, role);
          getRealmResource()
  ...
  private RoleRepresentation createRoleRepresentation(String clientId, Role role) {
